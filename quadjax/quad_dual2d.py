...
class Quad2D(environment.Environment):
    ...
    def step_env(
        ...
        # thrust = [(a + 1.0) / 2.0 * params.max_thrust for a in action]
        # tau = [a * params.max_torque for a in action]
        thrust1 = (action[0] + 1.0) / 2.0 * params.max_thrust
        tau1 = action[1] * params.max_torque
        thrust2 = (action[2] + 1.0) / 2.0 * params.max_thrust
        tau2 = action[3] * params.max_torque
    ...
    def reset_env(
        ...
        state = EnvState(
            ...
            y2=y2, z2=z2, theta2=theta2, last_thrust2=0.0,last_tau2=0.0,
            y2_dot=0.0, z2_dot=0.0, theta2_dot=0.0,
            ...
            phi=0.0,phi_dot=0.0,
            phi2=0.0,phi2_dot=0.0,
            y_hook=y_hook,z_hook=z_hook,y_hook_dot=0.0,z_hook_dot=0.0,
            y_hook2=y_hook2,z_hook2=z_hook2,y_hook2_dot=0.0,z_hook2_dot=0.0,
            ...
            f_rope2=0.0, f_rope2_y=0.0, f_rope2_z=0.0,l_rope2=params.l,
        )
    ...
    def get_obs(self, state: EnvState, params: EnvParams) -> chex.Array:
        ...
        obs_elements = [
            ...
            state.y_hook2,
            state.z_hook2,
            state.y_hook2_dot / 4.0,
            state.z_hook2_dot / 4.0,
            state.y2,
            state.z2,
            state.theta2,
            state.y2_dot / 4.0,
            state.z2_dot / 4.0,
            state.theta2_dot / 40.0,
            state.y_tar - state.y2,
            state.z_tar - state.z2,
            (state.y_dot_tar - state.y2_dot) / 4.0,
            (state.z_dot_tar - state.z2_dot) / 4.0,
            state.phi2,
            state.phi2_dot / 10.0,
        ]
    ...
    def update_plot(frame_num):
        ...
        for i in range(start, frame_num + 1):
            ...
            plt.arrow(
                state_seq[i].y2,
                state_seq[i].z2,
                -0.1 * jnp.sin(state_seq[i].theta2),
                0.1 * jnp.cos(state_seq[i].theta2),
                width=0.01,
                color="b",
                alpha=alpha,
            )
            ...
            # plot hook as cross
            plt.plot(state_seq[i].y_hook2, state_seq[i].z_hook2, "x", color="r", alpha=alpha)
