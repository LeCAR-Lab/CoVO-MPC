...
class MPPIZejiSplineController(controllers.BaseController):
    def __init__(self, env, control_params, N: int, h: int, lam: float, n:int, expansion_mode:str = 'lqr') -> None:
        ...
        if expansion_mode == 'mean':
            ...
            def get_sigma_zeji(control_params, env_state, env_params, key):
                ...
                R = self.get_dJ_du(env_state, env_params, control_params, a_mean_waypoint, key)
            ...
            if expansion_mode == 'mppizeji':
                expansion_control_params = controllers.MPPIZejiSplineParams(
                    gamma_mean = 1.0,
                    gamma_sigma = 0.0,
                    discount = 1.0,
                    sample_sigma = 0.5, 
    ...
    def get_sigma_from_R(self, R: jnp.ndarray, control_params: MPPIZejiSplineParams):
        R = (R + R.T)/2.0
    ...
    def get_dJ_du(self, env_state:EnvState2D, env_params:EnvParams2D, control_params:MPPIZejiSplineParams, a_mean:jnp.ndarray, rng_act: chex.PRNGKey = None):
        ...
        def get_cumulated_cost(a_mean_flattened, carry):
            a_mean_waypoint = a_mean_flattened.reshape(self.h, -1)
            def linear_interpolation(a, b):
                return jnp.arange(self.n) * (b - a) / self.n + a
            a_mean_original = jax.vmap(jax.vmap(linear_interpolation))(a_mean_waypoint[:-1], a_mean_waypoint[1:]) # (H-1, action_dim, n)
            a_mean_transposed = jnp.transpose(a_mean_original, (0, 2, 1)) # (H-1, n, action_dim)
            a_mean = jnp.reshape(a_mean_transposed, ((self.h-1)*self.n, -1)) # ((H-1)*n, action_dim)
            a_mean = jnp.concatenate([a_mean, a_mean_waypoint[-1:]], axis=0) # ((H-1)*n+1, action_dim)
    ...
    def __call__(self, obs:jnp.ndarray, env_state, env_params, rng_act: chex.PRNGKey, control_params: MPPIZejiSplineParams, info = None) -> jnp.ndarray:
        ...
        def single_sample(key):
            return jax.random.multivariate_normal(key, a_mean_waypoint.flatten(), control_params.a_cov)
        # repeat single_sample N times to get N samples
        # get waypoint prior from last control by get point every n points
        a_sampled_waypoint_flattened = jax.vmap(single_sample)(act_keys)
        a_sampled_waypoint = a_sampled_waypoint_flattened.reshape(self.N, self.h, -1)
        def linear_interpolation(a, b):
            return jnp.arange(self.n) * (b - a) / self.n + a
