...
class CoVOController(controllers.BaseController):
    def __init__(self, env, control_params, N: int, H: int, lam: float, mode:str = 'online') -> None:
        ...
        if mode == 'online':
            ...
            def get_sigma_zeji(control_params, env_state, env_params, key):
                R = self.get_hessian(env_state, env_params, control_params, control_params.a_mean, key)
    ...
    def get_hessian(self, env_state:EnvState3D, env_params:EnvParams3D, control_params:CoVOParams, a_mean:jnp.ndarray, rng_act: chex.PRNGKey = None):
    ...
    def __call__(self, obs:jnp.ndarray, env_state, env_params, rng_act: chex.PRNGKey, control_params: CoVOParams, info = None) -> jnp.ndarray:
