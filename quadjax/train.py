    ...
    def train(rng):
        ...
        if config["enable_curri"]:
            ...
            env_params = env_params.replace(
                curri_params=jnp.ones(config["NUM_ENVS"]) * curri_params
            )

        # INIT NETWORK
        network = ActorCritic(env.action_dim, activation=config["ACTIVATION"])
        ...
        if config["enable_RMA"]:
            ...
            def get_pi_value(train_params, last_obs, env_info):
                compressed_params_obs = compressor.apply(
        ...
        def _env_step(get_pi_value, runner_state, unused):
            ...
            pi, value = get_pi_value(
                get_network_params(ppo_train_state, adapt_train_state),
                last_obs,
                env_info,
            )
            ...
            if config["enable_curri"]:
                new_env_params = new_env_params.replace(
                    curri_params=jnp.ones(config["NUM_ENVS"]) * curri_params
                )
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(ppo_train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        # RERUN NETWORK
                        pi, value = get_pi_value(
                            params, traj_batch.obs, traj_batch.info
                        )
        ...
        def _train_adaptor(runner_state, unused):
            ...
            def _update_adaptor(update_state, unused):
                def _update_minbatch(adapt_train_state, batch_info):
                    ...
                    def _loss_fn(
                        adaptor_params, compressor_params, obs_param, obs_adapt
                    ):
                        # RERUN NETWORK
                        compressor_value = compressor.apply(
                            compressor_params, obs_param
                        )
                    ...
                    total_loss, grads = grad_fn(
                        adapt_train_state.params,
                        ppo_train_state.params[1],
                        obs_param,
                        obs_adapt,
                    )
        ...
        metric = {
            ...
            "mean_episode_returns": jnp.array([]),
            "err_pos": jnp.array([]),
            "err_vel": jnp.array([]),
            "err_pos_last_10": jnp.array([]),
            "final_reward": jnp.array([]),
        }
        step_per_log = config["NUM_STEPS"] * config["NUM_ENVS"]
        for i in range(config["NUM_PPO_UPDATES"]):
            runner_state, metric_local = jax.block_until_ready(
                _train_ppo(runner_state, None)
            )

            metric_log = {}
            metric_log["step"] = jnp.array([(i + 1) * step_per_log])
            metric_log["mean_episode_returns"] = jnp.mean(
                metric_local["returned_episode_returns"][-1]
                / (metric_local["returned_episode_lengths"][-1] + 1.0)
            )
            metric_log["returned_episode_returns"] = metric_local[
                "returned_episode_returns"
            ][-1].mean()
            metric_log["returned_episode_lengths"] = metric_local[
            ...
            final_step_mask = jnp.concatenate(
                [
                    metric_local["returned_episode"][1:],
                    jnp.zeros_like(metric_local["returned_episode"][-1:]),
                ],
                axis=0,
            )
        ...
        if config["enable_RMA"]:
            ...
            for i in range(config["NUM_ADAPT_UPDATES"]):
                runner_state, metric_local = jax.block_until_ready(
                    _train_adaptor(runner_state, None)
                )

                metric_log = {}
                metric_log["step"] = jnp.array(
                    [(i + config["NUM_PPO_UPDATES"] + 1) * step_per_log]
                )
                metric_log["mean_episode_returns"] = jnp.mean(
                    metric_local["returned_episode_returns"][-1]
                    / (metric_local["returned_episode_lengths"][-1] + 1.0)
...
class Args:
    ...
    env: str = "quadrotor"  # quad2d_free, quad2d, quad3d_free, quad3d
...
def main(args: Args):
    ...
    if args.env == "quadrotor":
        env = quadjax.envs.quadrotor.Quad3D(
            task=args.task,
            obs_type=args.obs_type,
            enable_randomizer=(not args.noDR),
            lower_controller=args.lower_controller,
            disturb_type=args.disturb_type,
            disable_rollover_terminate=False, 
        )
        render_fn = quadjax.envs.quadrotor.render_env
        eval_fn = quadjax.envs.quadrotor.eval_env
    else:
    ...
    plt.savefig(
        f"{quadjax.get_package_path()}/../results/training_curve_{filename}.png"
    )
    ...
    with open(
        f"{quadjax.get_package_path()}/../results/ppo_params_{filename}.pkl", "wb"
    ) as f:
