...
def get_taut_dynamics():
    ...
    y_ddot, z_ddot, theta_ddot, phi_ddot = (
        sp.diff(y_dot, t),
        sp.diff(z_dot, t),
        sp.diff(theta_dot, t),
        sp.diff(phi_dot, t),
    ...
    eq_quad_theta = (
        tau + delta_yh_global * f_rope_z - delta_zh_global * f_rope_y - I * theta_ddot
    )
    ...
    eqs = [
        eq.subs([(states_dot[i], states_dot_val[i]) for i in range(len(states_dot))])
        for eq in eqs
    ]
    eqs = [
        eq.subs([(states[i], states_val[i]) for i in range(len(states))]) for eq in eqs
    ]
    ...
    obs_eqs = [
        eq.subs([(states[i], states_val[i]) for i in range(len(states))])
        for eq in obs_eqs
    ]
    # lambda obs_eqs
