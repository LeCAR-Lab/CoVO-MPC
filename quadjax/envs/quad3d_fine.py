...
class Quad3D(BaseEnvironment):
    ...
    def __init__(
        ...
        if dynamics == "free":
            ...
            self.step_fn, self.update_time = quad_dyn.get_free_dynamics_pwm()
            self.get_err_pos = lambda state: jnp.linalg.norm(state.pos_tar - state.pos)
            self.get_err_vel = lambda state: jnp.linalg.norm(state.vel_tar - state.vel)
        ...
        if lower_controller == "base":
            ...
            self.sub_steps = 5
        elif lower_controller == "pid_pwm":
            self.default_control_params = controllers.PIDParamsPWM(
                kp=jnp.array([50.0, 50.0, 20.0]),
                ki=jnp.array([5.0, 5.0, 2.0]),
                kd=jnp.array([0.3, 0.3, 0.1]),
                last_error=jnp.zeros(3),
                integral=jnp.zeros(3),
                integral_max=jnp.array([1.0, 1.0, 1.0]),
            )
            controller = controllers.PIDControllerPWM(
                self, self.default_control_params
            )

            def pid_controller_fn(obs, state, env_params, rng_act, input_action):
                thrust_normed = input_action[:1]
                thrust = (thrust_normed + 1.0) / 2.0 * self.default_params.max_thrust
                # jax.debug.print('thrust normed = {x}', x = thrust / env_params.m / env_params.g)
                omega_tar = input_action[1:] * self.default_params.max_omega
                state = state.replace(omega_tar=omega_tar)
                torque, control_params, _ = controller(
                    obs, state, env_params, rng_act, state.control_params
                )

                # thrust = jnp.array([0.5067/2.0])
                # torque = jnp.array([0.00, 0.00, 0.001])
                # jax.debug.print('torque desired = {x}', x = torque/env_params.max_torque)

                state = state.replace(control_params=control_params)

                # convert thrust and torque to force
                eta = jnp.concatenate([thrust, torque])
                f = jnp.linalg.inv(quad_dyn.B0) @ eta
                # make sure force > 0
                ...
                # convert pwm to action
                action = (pwm / 65535 * 2.0) - 1.0
...
def render_env(env: Quad3D, controller, control_params, repeat_times=1, filename=""):
    ...
    while n_dones < repeat_times:
        ...
        if 'state_seq' in info:
            lower_level_state_seq.append(info['state_seq'])
        if done:
            rng, rng_params = jax.random.split(rng)
            env_params = env.sample_params(rng_params)
            rng, rng_control = jax.random.split(rng)
    ...
    if len(lower_level_state_seq) > 0:
        # create dataframe
        lower_level_state_seq_dict = [s.__dict__ for s in lower_level_state_seq]
        concate_dict = {}
        # concatenate all elements in the list
        for k, v in lower_level_state_seq_dict[0].items():
            if isinstance(v, jnp.ndarray):
                if len(v.shape) == 1:
                    concate_dict[k] = jnp.concatenate([d[k] for d in lower_level_state_seq_dict])
                elif len(v.shape) == 2:
                    for j in range(v.shape[1]):
                        concate_dict[f"{k}_{j}"] = jnp.concatenate(
                            [d[k][:, j] for d in lower_level_state_seq_dict]
                        )
        # convert to dataframe
        lower_level_state_seq_df = pd.DataFrame(concate_dict)
        # save to csv
        lower_level_state_seq_df.to_csv(
            f"{quadjax.get_package_path()}/../results/state_seq_{filename}.csv", index=False
        )
...
@pydataclass
class Args:
    task: str = "tracking_slow"  # tracking, tracking_zigzag, hovering
    dynamics: str = "pwm"  # bodyrate, free, slung
    controller: str = "pid"  # fixed
    controller_params: str = ""
    obs_type: str = "quad"
    debug: bool = False
    mode: str = "render"  # eval, render
    lower_controller: str = "pid_pwm"
    noDR: bool = False
    disturb_type: str = "none"  # periodic, sin, drag, gaussian, none
    name: str = ""
