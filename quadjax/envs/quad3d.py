...
class Quad3D(environment.Environment):
    ...
    def __init__(self, task: str = "tracking", lower_controller: str = 'base'):
        ...
        if lower_controller == 'base':
            ...
            self.substeps=1
        # lower-level controller
        elif lower_controller == 'base_compensate':
            self.default_control_params = 0.0
            def base_compensate_controller_fn(obs, state, env_params, rng_act, input_action):
                # compensate the torque generated by the rope
                torque_obj = jnp.cross(env_params.hook_offset, -env_params.g * env_params.mo * jnp.array([0.0, 0.0, 1.0]))
                torque_obj_normed = torque_obj / env_params.max_torque
                input_action = jnp.concatenate([input_action[:1], input_action[1:] - torque_obj_normed])
                return input_action, None, state
            self.control_fn = base_compensate_controller_fn
            self.substeps=1
...
def main(args: Args):
    ...
    if args.controller == 'mppi':
        ...
        thrust_hover = (env.default_params.m+env.default_params.mo) * env.default_params.g
        thrust_hover_normed = (thrust_hover / env.default_params.max_thrust) * 2.0 - 1.0
