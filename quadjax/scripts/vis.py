...
vis["obj_tar"].set_object(
    g.Sphere(0.02), material=g.MeshLambertMaterial(color=0xFF0000)
)
...
for i, state in enumerate(state_seq):
    if i % 20 == 0:
        ...
        for j in range(0, 300, 2):
            set_frame(
                i,
                f"traj{j}",
                pos_quat_to_transform(
                    state_seq[i]["pos_traj"][j], np.array([0, 0, 0, 1])
                ),
            )
    ...
    set_frame(
        i, "obj_tar", pos_quat_to_transform(state["pos_tar"], np.array([0, 0, 0, 1]))
    )
    set_frame(
        i, "disturb", origin_vec_to_transform(state["pos"], state["f_disturb"], 2.0)
    )
